...
struct CodableArchiverClassTests {
  ...
  func archiveClassArray() throws {
    ...
    let decoded = archived.compactMap { data in
      try? archiver.decoder.decode(TestCodableClass<String>.self, from: data as Data)
    }
  ...
  func archiveClassContainingStructArray() throws {
    ...
    let decoded = archived.compactMap { data in
      try? archiver.decoder.decode(
        TestCodableClass<TestCodableStruct<String>>.self, from: data as Data)
    }
    #expect(decoded == values)
    try? archiver.clear()
  }
