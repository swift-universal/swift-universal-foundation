...
- Lives under the `JSON` namespace (SwiftUniversalMain) and is implemented in SwiftUniversalFoundation.
...
Protocols for pluggable JSON encoding/decoding. Higher layers (e.g., SwiftUniversalFoundation) can supply
...
## Foundation Defaults (SwiftUniversalFoundation)

- `JSONEncoder` (SwiftUniversalFoundation default implementation of `JSONDataEncoding`)
- `JSONDecoder` (SwiftUniversalFoundation default implementation of `JSONDataDecoding`)
- Presets: `JSONEncoder.commonDateFormatting`, `JSONDecoder.commonDateParsing`

## Create a Parser

```swift
import SwiftUniversalFoundation  // brings JSON.Parser + default conformances
...
## Can More Types Move into SwiftUniversalMain?

Currently, the protocol types already live in SwiftUniversalMain. The `JSON.Parser` implementation depends on Foundation types (`Data`) and default encoder/decoder presets supplied by SwiftUniversalFoundation, so it remains in SwiftUniversalFoundation. If we split out only the bare struct (without presets) and accept that SwiftUniversalMain depends on Foundation, the core could be moved later — but the present layout keeps concerns tidy: defaults and Foundation helpers together in SwiftUniversalFoundation.
